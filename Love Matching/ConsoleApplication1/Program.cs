...
namespace ConsoleApplication1
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("以下是您的資料: ");
            Console.WriteLine(SHE + " : " + name[SHE] + " " + id[SHE] + " " + height[SHE] + " " + gender[SHE]);

            for (int a = 0; a < 5; a++)
            {
                Console.Clear();
                Console.WriteLine("正在幫您找尋合適的伴侶...");
                System.Threading.Thread.Sleep(500);
            }


            Console.WriteLine("找尋成功！以下是最適合您的配偶！");
            Console.WriteLine("");
            ...
            if (gender[SHE] == "男")
            {
                ...
                for (int i = 0; i < id.Length; i++)
                {
                    if (height[i] < heightperfecthigh && height[i] > heightperfectlow && gender[i] == "女")
            ...
            else
            {
                ...
                for (int i = 0; i < id.Length; i++)
                {
                    if (height[i] < heightperfecthigh && height[i] > heightperfectlow && gender[i] == "男")
                        Console.WriteLine(i + " " + name[i] + " " + height[i]);
                }
            }
            Console.WriteLine("配對成功，祝你幸福，歡迎再次使用！");
            Console.ReadKey();
        }
    }
}
